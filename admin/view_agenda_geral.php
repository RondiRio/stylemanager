...
// 1. Base da Query com COALESCE para garantir que o nome apareça
$query = "
    SELECT
        ...
        COALESCE(u.nome, cr.nome, a.cliente_nome) AS cliente_nome,
        COALESCE(u.telefone, cr.telefone, a.cliente_telefone) AS cliente_telefone,
    ...
    LEFT JOIN clientes_rapidos cr ON cr.id = a.cliente_rapido_id
...
// 3. Filtro de Busca (Nome ou Telefone nas 3 fontes possíveis)
if (!empty($busca_cliente)) {
    $query .= " AND (
        u.nome LIKE ? OR u.telefone LIKE ? OR 
        cr.nome LIKE ? OR cr.telefone LIKE ? OR 
        a.cliente_nome LIKE ? OR a.cliente_telefone LIKE ?
    )";
    $termo = "%{$busca_cliente}%";
    // Adiciona o termo 6 vezes para cobrir todos os '?' do bloco acima
    $params = array_merge($params, array_fill(0, 6, $termo));
}
// 4. Agrupamento e Ordenação (Sempre no final da query)
$query .= " GROUP BY a.id ORDER BY a.hora_agendamento";
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3 border-end">
                                <div class="d-flex align-items-center gap-3 mb-3">
                                    ...
                                    <div>
                                        <h6 class="mb-1 fw-bold"><?php echo htmlspecialchars($agendamento['cliente_nome']?? 'não informado'); ?></h6>
                                        <?php
                                        ?>
                            ...
                            <div class="col-md-5 border-end">
                                <h6 class="text-primary mb-3">
                                    ...
                                    <?php echo substr($agendamento['hora_agendamento'], 0, 5); ?>
                            ...
                            <div class="col-md-2 d-flex flex-column gap-2 justify-content-center">
                                <?php if ($agendamento['status'] !== 'cancelado' && $agendamento['status'] !== 'finalizado'): ?>
                                    ...
                                    <div class="dropdown">
                                        <?php 
                                        // Verifica se a data visualizada é hoje
$hoje = date('Y-m-d');
$pode_alterar = ($data === $hoje);
                                        ?>
                                        <button class="btn btn-primary btn-sm dropdown-toggle w-100" type="button"
        data-bs-toggle="dropdown" aria-expanded="false"
        <?php echo !$pode_alterar ? 'disabled title="Alterações permitidas apenas no dia do atendimento"' : ''; ?>>
